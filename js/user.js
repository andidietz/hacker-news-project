...
async function toggleFavorite(evt) {
  ...
  if (evt.target.tagName === 'BUTTON') {    
    if (evt.target.classList.contains("saved-favorite")) {
      const storyId = evt.target.parentElement.id;
      await User.deleteFavorite(storyId);

      evt.target.classList.toggle("saved-favorite");
    } else {
      const storyId = evt.target.parentElement.id;
...
async function putFavoritesOnPage() {
  ...
  for (let favorite of currentUser.favorites) {
    ...
    $favorite[0].children[4].classList.add("saved-favorite");

  }
  $allFavoritesList.show();
  $allFavoritesList.on("click", toggleFavorite);
}
